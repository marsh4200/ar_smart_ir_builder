...
TEST_SCHEMA = vol.Schema(
    {
        ...
        vol.Required("device_key"): cv.string,
        vol.Required("command_name"): cv.string,
        vol.Optional("num_repeats"): vol.All(vol.Coerce(int), vol.Range(min=1, max=20)),
        vol.Optional("delay_ms"): vol.All(vol.Coerce(float), vol.Range(min=0, max=60000)),
    }
)
...
def _build_ha_scripts_export(
    ...
    lines.append("# Generated by AR Smart IR Builder")
        ...
        async_register_built_in_panel(
            ...
            config={
                "_panel_custom": {
                    ...
                    "js_url": f"/api/{DOMAIN}/static/panel.js?v=33",
                }
            },
            require_admin=True,
        )
...
def _async_register_services(hass: HomeAssistant) -> None:
    ...
    async def learn(call: ServiceCall) -> dict[str, Any]:
        ...
        remote_state = hass.states.get(remote)
        if remote_state is None:
            raise HomeAssistantError(
                f"Remote entity '{remote}' does not exist. Check that the "
                "Broadlink integration is set up and pick the correct remote "
                "in this entry's options."
            )
        if remote_state.state in ("unavailable", "unknown"):
            raise HomeAssistantError(
                f"Remote entity '{remote}' is {remote_state.state}. The "
                "Broadlink device looks offline — check its power and network, "
                "then try again."
            )

        mode = call.data.get("command_type", "ir")
        ...
        try:
            ...
            hint = (
                " For RF: press and HOLD the button during the frequency sweep, "
                "then TAP it once when the Broadlink LED changes. RF learning "
                "needs an RM Pro / RM4 Pro."
                if mode == "rf"
                else " Make sure the selected entity is a Broadlink remote and "
                "you pressed the button while the LED was on."
            )
            raise HomeAssistantError(
        ...
        if code is None:
            hint = (
                ...
                "sweep finishes, then TAP it once to capture. If it keeps "
                "failing, the remote may use a rolling code (e.g. Somfy RTS), "
                "which a Broadlink cannot replay."
                if mode == "rf"
                else "Point the remote directly at the Broadlink and press the "
                "button once while its LED is on."
            )
            raise HomeAssistantError(
                f"No new code appeared in Broadlink storage — the capture "
                f"didn't complete. {hint}"
            )
    ...
    async def test_command(call: ServiceCall) -> dict[str, Any]:
        ...
        # Repeat policy: use the profile's per-command policy (same one the
        # real entities use via send_with_policy) so Test behaves like the
        # entity will. Explicit num_repeats/delay_ms in the call overrides.
        repeat, delay = resolve_repeat_policy(device, command_name)
        if "num_repeats" in call.data:
            repeat = int(call.data["num_repeats"])
        if "delay_ms" in call.data:
            delay = float(call.data["delay_ms"]) / 1000.0
        repeat = max(1, min(repeat, 20))
        delay = max(0.0, min(delay, 60.0))

        code_type = _classify_broadlink_code(code)
        ...
        remote_state = hass.states.get(remote)
        if remote_state is None or remote_state.state in ("unavailable", "unknown"):
            raise HomeAssistantError(
                f"Remote entity '{remote}' is missing or offline — check the "
                "Broadlink device, then try again."
            )
        ...
        try:
            ...
            raise HomeAssistantError(f"Broadlink send failed: {err}") from err
...
def _classify_broadlink_code(code: str) -> str:
    """Best-effort IR/RF classification from the Broadlink packet type byte.

    Broadlink base64 payloads start with 0x26 (IR), 0xB2 (RF 433 MHz) or
    0xD7 (RF 315 MHz). Used only for user-facing hints — never for gating.
    """
    try:
        import base64

        first = base64.b64decode(code + "===")[0]
...
async def get_last_code(
    ...
    while True:
        ...
        if asyncio.get_running_loop().time() >= deadline:
            # Do NOT fall back to a pre-existing code here. Returning a stale
            # baseline code silently saved the WRONG signal under the new
            # command name ("learning worked, testing does nothing").
            return None
